...
// Adjusted package from `sha3` to `crypt`, removed internal cShake references
...
func newKMAC(key []byte, tagSize int, customizationString []byte, c sha3.ShakeHash, blockSize int) hash.Hash {
	...
	k.Write(bytepad(k.initBlock, k.BlockSize())) //nolint:errcheck
	return k
}
...
func (k *kmac) Sum(b []byte) []byte {
	...
	dup.Write(rightEncode(uint64(k.tagSize * 8))) //nolint:errcheck
	hash := make([]byte, k.tagSize)
	dup.Read(hash) //nolint:errcheck
